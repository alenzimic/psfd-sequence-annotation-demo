...
GENE_PROTEIN_NORMALIZATION_DIR = SOURCE_ROOT / "output" / "10_gene_protein_normalization" / "940_gene_protein_normalization"
CONTEXT_PROPAGATION_DIR = SOURCE_ROOT / "output" / "11_context_propagation"
PORTABLE_PHYTOZOME_FASTA_DIR = SOURCE_ROOT / "data" / "phytozome" / "raw"
...
PHYTOZOME_FASTA_CACHE: dict[str, dict[str, dict[str, str]]] = {}
...
def relpath(path: str | Path | None) -> str:
    ...
    try:
        return str(path_obj.resolve().relative_to(SOURCE_ROOT))
    except (ValueError, OSError):
        return str(path).replace(str(SOURCE_ROOT) + "/", "")
...
def phytozome_fasta_records(source_file: str) -> dict[str, dict[str, str]]:
    """Parse a Phytozome protein FASTA file once and expose records by primary ID."""
    source_name = Path(str(source_file or "")).name
    if not source_name:
        return {}
    if source_name in PHYTOZOME_FASTA_CACHE:
        return PHYTOZOME_FASTA_CACHE[source_name]

    fasta_path = PHYTOZOME_FASTA_DIR / source_name
    records: dict[str, dict[str, str]] = {}
    if not fasta_path.exists():
        PHYTOZOME_FASTA_CACHE[source_name] = records
        return records
    ...
    def flush_record() -> None:
        ...
        if primary_id and sequence:
            records.setdefault(
                primary_id,
                {
                    "fasta_header": header,
                    "sequence": sequence,
                    "sequence_source_file": source_name,
                },
            )
...
def phytozome_sequence_record(phytozome: dict[str, Any]) -> dict[str, str]:
    gene_id = str(phytozome.get("gene_id") or "")
    code = str(phytozome.get("code") or "")
    source_file = str(phytozome.get("source_file") or "")
    if not gene_id or not source_file:
        return {}
    records = phytozome_fasta_records(source_file)
    candidate_ids = [gene_id]
    if code:
        candidate_ids.append(f"{code}_{gene_id}")
    for candidate_id in candidate_ids:
        if candidate_id in records:
            return {**records[candidate_id], "fasta_id": candidate_id}

    suffix = f"_{gene_id}"
    suffix_matches = [key for key in records if key.endswith(suffix)]
    if len(suffix_matches) == 1:
        match = suffix_matches[0]
        return {**records[match], "fasta_id": match}
    return {}
            ...
            "phytozome": compact_dict(
                {
                    ...
                    "source_file": relpath(raw.get("selected_phytozome_source_file", "")),
                    "source_database": raw.get("source_database", ""),
                }
            ),
            "family": compact_dict(
...
def build_gene_protein_profile(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in sorted_rows:
        ...
        if phytozome.get("gene_id"):
            ...
            sequence_record = phytozome_sequence_record(phytozome)
            phytozome_ids.setdefault(
                gene_id,
                {
                    ...
                    "sequence_available": bool(sequence_record.get("sequence")),
                    "fasta_id": sequence_record.get("fasta_id", ""),
                    "fasta_header": sequence_record.get("fasta_header", ""),
                    "sequence": sequence_record.get("sequence", ""),
                    "sequence_source_file": sequence_record.get("sequence_source_file", ""),
                },
            )
...
def compact_sources(source_files: dict[str, Any]) -> dict[str, str]:
    return {key: relpath(value) for key, value in source_files.items()}
...
def build_paper(pmcid: str) -> dict[str, Any]:
    ...
    stats = {
        ...
        "gene_protein_phytozome_fasta_entities": sum(
            1
            for row in entities_by_id.values()
            if any(
                item.get("sequence_available")
                for item in (row.get("gene_protein_normalization") or {}).get("phytozome_ids", [])
            )
        ),
...
def main() -> None:
    ...
    CONTEXT_PROPAGATION_DIR = SOURCE_ROOT / "output" / "11_context_propagation"
    PORTABLE_PHYTOZOME_FASTA_DIR = SOURCE_ROOT / "data" / "phytozome" / "raw"
    PHYTOZOME_FASTA_DIR = default_resource_path("PSFD_PHYTOZOME_DIR", PORTABLE_PHYTOZOME_FASTA_DIR)

    manifest = build(args.outdir.resolve())
    print(f"Read PSFD outputs from {SOURCE_ROOT}")
    print(f"Wrote PSFD static data to {args.outdir.resolve() / 'data'}")
